...
function getCommuneFromCadastre(codeCommune, prefixeSection) {
  ...
  const recente = getMostRecentCommune(codeCommune)
  if (!recente) return null

  const codes2 = getCodesMembres(recente)
  const found = codes2.find(c => c.endsWith(prefixeSection))
  if (!found) return null

  return getOlderCommune(found)
}

function getMostRecentCommune(codeCommune) {
  const entries = byCodeCommune[`COM${codeCommune}`]
  if (!entries) return null
  return maxBy(entries, c => c.dateFin || '9999-99-99')
}
...
function getOlderCommune(codeCommune) {
  ...
  const entries = byCodeCommune[`COM${codeCommune}`]
  if (!entries) return null
  return minBy(entries, c => c.dateDebut || '0000-00-00')
}

function getCommune(codeCommune, dateValeur, types = ['COM', 'COMD', 'COMA', 'COMP']) {
  if (!codeCommune) {
    throw new Error(`Code commune vide. Date de valeur : ${dateValeur}`)
  }
...
function getCommuneActuelle(communeEntry, dateValeur) {
  ...
  if (isValidAt(communeEntry, dateValeur)) {
    return communeEntry.pole ? getCommuneActuelle(communeEntry.pole, dateValeur) : null
  }
